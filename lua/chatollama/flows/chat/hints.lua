...
function M.get_panel()
  ...
  M.panel = Popup({
    ...
    win_options = {
      winhighlight = "Normal:ChatOllamaHintsBar,FloatBorder:ChatOllamaHintsBar",
    },
    buf_options = {
      buftype = "nofile",
      swapfile = false,
...
function M.render(context)
  ...
  local virt_text = {}
  if padding > 0 then
    table.insert(virt_text, { string.rep(" ", padding), "ChatOllamaHintsBar" })
  end
  for i, h in ipairs(hints) do
    table.insert(virt_text, { h.key, "ChatOllamaHintsKey" })
    table.insert(virt_text, { " " .. h.label, "ChatOllamaHintsText" })
    if i < #hints then
      table.insert(virt_text, { " • ", "ChatOllamaHintsSep" })
    end
  end
