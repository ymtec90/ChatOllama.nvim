...
local Sessions = require("ChatOllama.flows.chat.sessions")
...
local SystemWindow = require("ChatOllama.flows.chat.system_window")
local Context = require("ChatOllama.context")
local LspContext = require("ChatOllama.context.lsp")
local ProjectContext = require("ChatOllama.context.project")
local Hints = require("ChatOllama.flows.chat.hints")
...
function Chat:init()
  ...
  self.selected_message_nsid = vim.api.nvim_create_namespace("ChatOllamaNSSM")
  ...
  if not self:is_compact() then
    if #self.session.conversation == 0 or (#self.session.conversation == 1 and self.system_message ~= nil) then
      ...
      for line_num = 0, #lines do
        self:add_highlight("ChatOllamaWelcome", line_num, 0, -1)
      end
    end
  end
  self:render_role()
...
function Chat:render_role()
  ...
  if self.current_tokens > 0 then
    table.insert(virt_text, { Config.options.chat.border_left_sign, "ChatOllamaTokensBorder" })
    table.insert(virt_text, { self.current_tokens .. " TOKENS", "ChatOllamaTokens" })
    table.insert(virt_text, { Config.options.chat.border_right_sign, "ChatOllamaTokensBorder" })
    table.insert(virt_text, { " " })
  end

  -- Add role
  table.insert(virt_text, { Config.options.chat.border_left_sign, "ChatOllamaTotalTokensBorder" })
  table.insert(virt_text, { string.upper(self.role), "ChatOllamaTotalTokens" })
  table.insert(virt_text, { Config.options.chat.border_right_sign, "ChatOllamaTotalTokensBorder" })
  table.insert(virt_text, { " " })
...
function Chat:show_message_selection()
  ...
  vim.api.nvim_buf_set_extmark(self.chat_window.bufnr, self.selected_message_nsid, msg.start_line, 0, {
    ...
    hl_group = "ChatOllamaSelectedMessage",
    hl_eol = true,
  })
      ...
      virt_text = {
        {
          ...
          "ChatOllamaMessageAction",
        },
        { " ", "ChatOllamaSelectedMessage" },
      },
      virt_text_pos = "right_align",
    })
...
function Chat:highlight_message_content(lines, start_line)
  ...
  for i, line in ipairs(lines) do
    ...
    if block_start and not in_code_block then
      ...
      local header_text = {}
      if code_lang then
        table.insert(header_text, { " LANGUAGE: " .. string.upper(code_lang) .. " ", "ChatOllamaCodeLang" })
        table.insert(
          header_text,
          { " ────────────────── ", "ChatOllamaCodeBlockHeader" }
        )
        table.insert(header_text, { "[y] copy", "Comment" })
      else
        table.insert(header_text, { " CODE ", "ChatOllamaCodeLang" })
        table.insert(
          header_text,
          { " ────────────────── ", "ChatOllamaCodeBlockHeader" }
        )
      ...
      vim.api.nvim_buf_add_highlight(bufnr, Config.namespace_id, "ChatOllamaCodeBlock", line_num, 0, -1)
    elseif in_code_block and line == "```" then
      in_code_block = false
      code_lang = nil
      -- Hide the closing ``` with a subtle separator
      vim.api.nvim_buf_set_extmark(bufnr, Config.namespace_id, line_num, 0, {
        virt_text = { { "───", "ChatOllamaCodeBlockHeader" } },
        virt_text_pos = "overlay",
        end_col = 3,
        conceal = "",
      })
    ...
    elseif in_code_block then
      ...
      if code_lang == "diff" then
        if line:match("^%+") then
          vim.api.nvim_buf_add_highlight(bufnr, Config.namespace_id, "ChatOllamaDiffAdd", line_num, 0, -1)
        elseif line:match("^%-") then
          vim.api.nvim_buf_add_highlight(bufnr, Config.namespace_id, "ChatOllamaDiffDel", line_num, 0, -1)
        else
          vim.api.nvim_buf_add_highlight(bufnr, Config.namespace_id, "ChatOllamaCodeBlock", line_num, 0, -1)
        end
      else
      ...
      if line:match("^#+%s") then
        vim.api.nvim_buf_add_highlight(bufnr, Config.namespace_id, "ChatOllamaHeader", line_num, 0, -1)
      -- Horizontal rule (--- or ***)
      elseif line:match("^%-%-%-+%s*$") or line:match("^%*%*%*+%s*$") then
        vim.api.nvim_buf_add_highlight(bufnr, Config.namespace_id, "ChatOllamaHRule", line_num, 0, -1)
      -- Blockquote (> text)
      elseif line:match("^>") then
        vim.api.nvim_buf_add_highlight(bufnr, Config.namespace_id, "ChatOllamaBlockquote", line_num, 0, -1)
      else
        -- Bullet list markers (- or *)
        local bullet = line:match("^(%s*[%-%*]%s)")
        if bullet then
          vim.api.nvim_buf_add_highlight(bufnr, Config.namespace_id, "ChatOllamaListMarker", line_num, 0, #bullet)
        end

        -- Numbered list markers (1. 2. etc)
        local num_marker = line:match("^(%s*%d+%.%s)")
        if num_marker then
          vim.api.nvim_buf_add_highlight(bufnr, Config.namespace_id, "ChatOllamaListMarker", line_num, 0, #num_marker)
        end
        ...
        for ref in line:gmatch("@[^%s]+") do
          ...
          if s then
            vim.api.nvim_buf_add_highlight(bufnr, Config.namespace_id, "ChatOllamaContextRef", line_num, s - 1, e)
          end
        end
        ...
        while true do
          ...
          vim.api.nvim_buf_add_highlight(bufnr, Config.namespace_id, "ChatOllamaInlineCode", line_num, s - 1, e)
          col = e + 1
        end
        ...
        while true do
          ...
          vim.api.nvim_buf_add_highlight(bufnr, Config.namespace_id, "ChatOllamaBold", line_num, s - 1, e)
          col = e + 1
        end
        ...
        while true do
          ...
          if line:sub(s, s + 1) ~= "**" and (s == 1 or line:sub(s - 1, s - 1) ~= "*") then
            vim.api.nvim_buf_add_highlight(bufnr, Config.namespace_id, "ChatOllamaItalic", line_num, s - 1, e)
          end
          col = e + 1
        end
        ...
        while true do
          ...
          vim.api.nvim_buf_add_highlight(bufnr, Config.namespace_id, "ChatOllamaLink", line_num, s - 1, e)
          col = e + 1
        end
        ...
        if task_done then
          vim.api.nvim_buf_add_highlight(bufnr, Config.namespace_id, "ChatOllamaTaskDone", line_num, 0, #task_done)
        end
        local task_pending = line:match("^(%s*%-%s*%[%s*%])")
        if task_pending then
          vim.api.nvim_buf_add_highlight(bufnr, Config.namespace_id, "ChatOllamaTaskPending", line_num, 0, #task_pending)
        end
        ...
        while true do
          ...
          vim.api.nvim_buf_add_highlight(bufnr, Config.namespace_id, "ChatOllamaStrikethrough", line_num, s - 1, e)
          col = e + 1
        end
        ...
        while true do
          ...
          if text_end then
            vim.api.nvim_buf_add_highlight(bufnr, Config.namespace_id, "ChatOllamaLinkText", line_num, s - 1, text_end)
            vim.api.nvim_buf_add_highlight(bufnr, Config.namespace_id, "ChatOllamaLinkUrl", line_num, text_end, e)
          end
          col = e + 1
        end
      end
...
function Chat:add_message_divider(line_num)
  ...
  vim.api.nvim_buf_set_extmark(bufnr, Config.namespace_id, line_num, 0, {
    virt_lines = { { { divider, "ChatOllamaDivider" } } },
...
function Chat:renderLastMessage()
  ...
  if msg.type == QUESTION then
    ...
    vim.api.nvim_buf_set_extmark(self.chat_window.bufnr, Config.namespace_id, msg.start_line, 0, {
      virt_text = { { "You ", "ChatOllamaSenderUser" } },
      virt_text_pos = "inline",
    })

    -- Apply base question styling
    for index, _ in ipairs(lines) do
      self:add_highlight("ChatOllamaQuestion", msg.start_line + index - 1, 0, -1)
    end
    ...
    -- Add sender indicator
    vim.api.nvim_buf_set_extmark(self.chat_window.bufnr, Config.namespace_id, msg.start_line, 0, {
      virt_text = { { "Assistant ", "ChatOllamaSenderAssistant" } },
      virt_text_pos = "inline",
    })
    ...
    if total_tokens ~= nil then
      self.messages[self.selectedIndex].extmark_id =
        vim.api.nvim_buf_set_extmark(self.chat_window.bufnr, Config.namespace_id, msg.end_line + 1, 0, {
          virt_text = {
            { Config.options.chat.border_left_sign, "ChatOllamaTotalTokensBorder" },
            {
              "TOKENS: " .. msg.usage.total_tokens,
              "ChatOllamaTotalTokens",
            },
            { Config.options.chat.border_right_sign, "ChatOllamaTotalTokensBorder" },
            { " ", "" },
          },
          virt_text_pos = "right_align",
        })
...
function Chat:open()
  ...
  local resize_group = vim.api.nvim_create_augroup("ChatOllamaResize", { clear = true })
...
function Chat:hide()
  ...
  if self.resize_group then
    vim.api.nvim_create_augroup("ChatOllamaResize", { clear = true })
  end
  self.layout:hide()
end
