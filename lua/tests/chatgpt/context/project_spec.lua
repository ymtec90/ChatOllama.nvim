local project = require("chatgpt.context.project")

describe("chatgpt.context.project", function()
  local original_vim
  ...
  before_each(function()
    ...
    package.loaded["chatgpt.config"] = {
      options = {
        context = {
          project = {
            context_files = { ".chatgpt.md", "custom.md" }
          }
        }
      }
    }
  ...
  describe("context files", function()
    ...
    it("find_context_file should return content if found", function()
      ...
      assert.are.same(".chatgpt.md", ctx.name)
